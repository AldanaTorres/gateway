...
export class DomicilioComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.domicilioService
      ...
      .subscribe(
        (res: HttpResponse<IDomicilio[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IDomicilio): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInDomicilios(): void {
    this.eventSubscriber = this.eventManager.subscribe('domicilioListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IDomicilio[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
